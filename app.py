...
# Isolated standard CSS rules - completely free from f-string template crashes
css_rules = """
<style>
...
def analyze_and_verify_claims(document_text):
    ...
    # FIXED: Replaced standard f-string wrapper with simple string concatenation to bypass bracket parsing syntax errors
    prompt = """
...
with col2:
    ...
    if uploaded_file and extracted_text:
        if st.button("🚀 Execute Automated Factcheck", type="primary"):
            ...
            """
            loading_placeholder.markdown(loading_info, unsafe_with_html=True)
            ...
            if parsed_claims:
                ...
                for idx, claim in enumerate(parsed_claims, 1):
                    ...
                    st.markdown(html_output, unsafe_with_html=True)
            else:
                if raw_response:
